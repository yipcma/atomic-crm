--
-- merge_contacts(loser_id, winner_id): consolidate the loser contact into the
-- winner (tasks, notes, deal membership, emails, phones, tags), then delete the
-- loser. Ported verbatim from supabase/schemas/02_functions.sql; uses only the
-- public schema, so it runs unchanged on a standalone Postgres.
--
